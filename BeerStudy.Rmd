...
####The following information comes from a study of craft beers in the U.S. A data set for 2410 craft beers and a data set fro 558 breweries were merged and used to provide interesting insights. Discussions include the analysis and code methods used in the analysis. The following will be reviewed and discussed. 
- Breweries by State
- Beer & Brewery Data Merge  
- Missing Data
- Median Alcohol By Volume (ABV) by State
- Median International Bitterness Units (IBU) by State
- Beers with the Highest ABV and IBU 
- ABV Summary
- ABV vs IBU 
- Next Steps

####First the two data sets are read into data frames beerDF and breweryDF from csv files. An RStudio project was created so the team could reference their own local Github working directory. The the columns (variables) of each data frame was custom named to be more readable and to provide a handle to merge the data later.
```{r echo = TRUE}
...
####The beer and brewery data frames were merged by BreweryID into data frame allBeerDataDF. kable from the kableExtra library was used to display nicely formated tables.
####The first and last 6 entries are shown as evidence of a successful merger. The last BreweryID is 558 and provides additional evidence of a successful merge since we know the brewery data set contianed information on 558 breweries.
```{r echo = TRUE}
...
####When munging data, there are often missing fields and there were no execptions here. The sapply function was used to sum the empty fields (reported as NA's) for each data column. The information was put into a data frame so kable could be used to display a nice table. The NA column was renamed as well.
...
####Plots were created to display the "median" AVB and IBU per State. The median function was used in the aggregate function to create the medianABV data frame. Beer with no ABVs were removed. The IBU data required and additional na.omit to remove South Dakota from the data frame since all IBUs for South Dakota were blank. ggplot was used to create horizontal bar plots and both used a rainbow gradient color scheme.
####The range for median AVBs is 4%-6%. Utah has the beer with the lowest median ABV at 4%. This is a very conservative state and 4% is the max ABV by law for packaged beers so this should be no surprise. DC, Kentucky, Missouri, and New Mexico round out the states with craft beers having the highest median AVBs.
```{r warning=FALSE, fig.align="center", echo=TRUE}
...
####A beer’s bitterness comes from the amount of hops brewers add to beer. Higher IBUs = more hoppy beer. The range for median IBUs roughly  20 to 60. Wisconsin, Kansas, and Arizona have craft beers with the lowest medain IBUs. Maine has the highest median IBU and West Virginia and Florida follow. Maine has the highest median IBU and West Virginia and Florida follow.
```{r warning=FALSE, fig.align="center", echo=TRUE}
...
####The which.max function was used within kable to display the beer and brewery information for the beer that had the highest ABV and highest IBU.
####Colorado has the beer with the highest ABV and Oregon and the beer with the highest IBU.
```{r warning=FALSE}
...
####The summary function was used with kable to display summary statistics for AVB. The data was transposed to display in a horizontal format. Then a boxplot is displayed which coincides with the AVB summary statistics. 
```{r}
...
text(x=1, y=.06,"mean ~ 0.06",col=4)                                                                    # add mean to box plot
```

####ggplot is once again used but this time to create a scatter plot of IBU vs ABV. Additionally, goem_smooth is used to show the linear fit for the data. The cor function is used to identify whether there is a statistically significant correlation with IBU and AVB. The code for this can be found in the Appendix section. The analysis used an alpha = 0.05. Below the plot, html tags and equation indicators are used to disply r and r-squared under the plot.
...
*This additional information would provide better guidance for craft beer investments*
